...
#import the data
data_dir = "/Users/dur-rbaral-m/projects/test_projects/data/jigsaw-multilingual-toxic-comment-classification"
...
def train(epoch):
    ...
    for index, data in enumerate(training_loader, 0):
        ...
        #if _ % 5 == 0:
        print(f'Epoch: {epoch}, Data: {index}  out of {len(training_loader)} Loss:  {loss.item()}')
...
def validation(epoch):
    ...
    fin_targets=[]
    fin_outputs=[]
    print("testing loader size ", len(testing_loader))
    with torch.no_grad():
        for index, data in enumerate(testing_loader, 0):
            print("validating item ",index," out of ",len(testing_loader))
            ids = data['ids'].to(device, dtype = torch.long)
            mask = data['mask'].to(device, dtype = torch.long)
            token_type_ids = data['token_type_ids'].to(device, dtype = torch.long)
            targets = data['targets'].to(device, dtype = torch.float)
